...
#-----------------------This is plant seedling dataset-----------------------------------------------
class PLANTDataProvider(DataProvider):
    """Data provider for plant seedling images."""

    def __init__(self, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, flatten=False):
        """Create a new PLANT Seedling data provider object.
        ...
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join(
            "data", 'plant-{0}.npz'.format(which_set))
        assert os.path.isfile(data_path), (
            'Data file does not exist at expected path: ' + data_path
        )
        # load data from compressed numpy file
    ...
    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(PLANTDataProvider, self).next()
        return inputs_batch, self.to_one_of_k(self.to_int_targets(targets_batch))
    ...
    def to_one_of_k(self, int_targets):
        ...
        one_of_k_targets[range(int_targets.shape[0]), int_targets] = 1
        return one_of_k_targets
    def to_int_targets(self, targets):
        """Converts string coded class target to int coded targets.
        """
        int_of_k_targets = np.array([CLASS[l] for l in targets])
        return int_of_k_targets
